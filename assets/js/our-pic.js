...
class Slider {
  ...
  next() {
    ...
    if (nextId > this.length) {
      nextId = 1;
    } else if (nextId == this.length) {
      setTimeout(() => {
        let responseEndPage2 = localStorage.getItem("responseEndPage2");

        if (!responseEndPage2) {
          const likePage2 = prompt("Khoái hongggg :>>");
          const wishPage2 = prompt(
            "Mong là những lời chúc của anh sẽ trở thành hiện thực, bởi vì em xứng đáng với những điều tốt đẹp nhất của cuộc đời, và cả của anh :> "
          );
          responseEndPage2 = {
            likePage2,
            wishPage2
          };
          localStorage.setItem("responseEndPage2", JSON.stringify(responseEndPage2));
        }
...
(() => {
  let responseStartPage2 = localStorage.getItem("responseStartPage2");

  if (!responseStartPage2) {
    const nextPage2 = prompt(
      "Tiếp đến là những lời chúc của anh, cũng như là những tấm hình tuyệt zời của hai đứa mình💓. Mong rằng em sẽ thích :>"
    );
    const startPage2 = prompt("Oke bắt đầu nào !!");
    responseStartPage2 = {
      nextPage2,
      startPage2,
    };

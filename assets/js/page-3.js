...
document.getElementById("confirm-btn").onclick = () => {
  let responsePage3 = localStorage.getItem("responsePage3");
  const wishes = document.getElementById("wishes").value;
  localStorage.setItem("wishes", wishes)
  if (!responsePage3) {
    const acceptance = prompt(
      "Đã ước rùi thì phải cố gắng hết sức để thực hiện nó á nhaa"
    );
    const cake = prompt(
      "okayy gooddd 😘😘 Bảo tặng Nhi cái bánh kem nèee, bữa đi ăn chưa có bánh kem nên tặng online đỡ nha, mốt dịch đỡ tụi mình đi ăn bùuu hihi (rê chuột vào người để thổi nến nháaa)"
    );
    responsePage3 = {
      acceptance,
      cake,
    };
    localStorage.setItem("responsePage3", JSON.stringify(responsePage3));
  }
